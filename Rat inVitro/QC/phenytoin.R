...
druggies <- list(ccl4, aspirin, valproicacid, azathioprine, cicloheximide, etoposide)

#'celvector <- rownames(subset(p, p$drugid_abbr=="NQO1"))

for (drug in druggies) {
  ...
  # subset mid
  if (length(drugTimePoints) < 21) {
    mid <- drugTimePoints[,c(5,13), drop = FALSE]
  } else {
    mid <- drugTimePoints[,c(5,13,21), drop = FALSE]
  }
  # subset high
  if (length(drugTimePoints) < 23) {
    high <- drugTimePoints[,c(7,15), drop = FALSE]
  } else {
  ...
  # subset mid
  if (length(drugTimePoints) < 22) {
    mid1 <- drugTimePoints[,c(6,14), drop = FALSE]
  } else {
    mid1 <- drugTimePoints[,c(6,14,22), drop = FALSE]
  }
  # subset high
  if (length(drugTimePoints) < 24) {
    high1 <- drugTimePoints[,c(8,16), drop = FALSE]
  } else {
  ...
  #plot
  time <- c(2,8,24)
  plot(time, control,lty = 2, pch = 16, col = "red", ylim = ylimit, xlim= c(2,26.5),type='n', axes=T, main = "Phenytoin on Rat Gene SLC26A1 (ENSRNOG00000000041)", cex.main = 0.8, xlab = "Time", ylab = "Expression", ann = T, xaxt = 'n', yaxt = 'n')
  
  # axes
  # x axis
  axis(side = 1, at = c(2,8,24), labels = c(2,8,24), tick = T, lty = "solid",lwd.ticks = 1, gap.axis = 0)
  # y axis
  axis(side = 2, at = c(9.5,10,10.5), labels = c(9.5,10,10.5), tick = T, lty = 1, col.ticks = T)
  ...
  # legend
  legend(22,10.53,legend=c("0 Ctrl CtrlA","0 Ctrl CtrlB","1 Low LowA","1 Low LowB","2 Mid MidA","2 Mid MidB","3 High HighA","3 High HighB"), col = c("red","red","green3","green3","blue","blue","cyan","cyan"),pch = c(16,17,3,4,23,25,22,21), pt.bg = c("red","red","green3","green3","blue","blue","cyan","cyan"),text.col = "black",lty = c(2,1,2,1,2,1,2,1,2),lwd = c(2,2,2,2,2,2,2,2), cex=0.3)
}

# subset dataframe columns specific to drug
#drugTimePoints <- subset(singleGeneRow, select = c(ccl4), drop = FALSE)
