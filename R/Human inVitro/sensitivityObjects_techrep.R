...
for (drug in uniqueDrugNames){ #iterate through each unique drug (set of experiments)
  ...
  #divide drugSamples by time -> 2, 8, 24h
  samples2<-subset(drugSamples,drugSamples$duration == "2 hr")
  samples8<-subset(drugSamples,drugSamples$duration == "8 hr")
  samples24<-subset(drugSamples,drugSamples$duration == "24 hr")
  
  #for each set of time-divided samples, check if all dose levels (low, med, high) are present;
  #YES -> computeSlope
  #NO -> nothing (leave as NA)
  if (NROW(samples2$dose_level) >= 3){
    samples2$slope_recomputed = computeSlope(samples2$concentration,samples2$DNA...)
    samples2$auc_recomputed = computeAUC(samples2$concentration, samples2$DNA...,conc_as_log = FALSE, viability_as_pct = TRUE, area.type = "Actual")
  }
  if (NROW(samples8$dose_level) >= 3){
    samples8$slope_recomputed = computeSlope(samples8$concentration,samples8$DNA...)
    samples8$auc_recomputed = computeAUC(samples8$concentration, samples8$DNA...,conc_as_log = FALSE, viability_as_pct = TRUE, area.type = "Actual")
  }
  if (NROW(samples24$dose_level) >= 3){
    samples24$slope_recomputed = computeSlope(samples24$concentration,samples24$DNA...)
    samples24$auc_recomputed = computeAUC(samples24$concentration, samples24$DNA...,conc_as_log = FALSE, viability_as_pct = TRUE, area.type = "Actual")
  }
  ...
  combinedSamples<-rbind(samples2,samples8)
  combinedSamples<-rbind(combinedSamples,samples24)
...
saveRDS(sensProf,"data/sensitivityProfiles_techrep.rds")
